...
void heap_insert(int a[], int &n, int val)
{
	...
	do
	{
		...
		if (a[i] <= a[parent])
			return;
		swap(a[i], a[parent]); // call the swap int
		i = parent;
	} while (i > 1);
}
...
int heap_delete(int a[], int &n)
{
	...
	do
	{
		...
		if (a[i] >= a[big_child])
			return result;
		swap(a[i], a[big_child]); // call the swap int
		i = big_child;
	} while (2 * i <= n);
	return result;
}
...
int main()
{
	...
	for (int i_repeat = 0; i_repeat < N_REPEAT; i_repeat++)
	{
		n_heap = 0;
		for (i = 0; i < n; i++)
			heap_insert(heap, n_heap, a[i+1]); // call the arm insert
		if (DEBUG && (i_repeat == 0))
		{
		...
		for (i = 0; i < n; i++)
		{
			if (DEBUG && (i_repeat == 0))
				if ((i > 0) && (i % 10 == 0))
					cout << endl;
			val = heap_delete(heap, n_heap); // call the arm delete
			sorted[i] = val;
			if (DEBUG && (i_repeat == 0))
				cout << val << " ";
		}
